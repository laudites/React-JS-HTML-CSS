...
//import TabelaNaoIniciado from '../naoIniciado/tabelaPC';

//import SalvarPedido from '../Pedido';
...
function BarraNavegacao() {
  ...
  //BarraLateral(showBarraLateral);
  let usuario = localStorage.getItem('user').toUpperCase();
  var primeiraLetraUsuario = usuario[0];
  ...
  useEffect(() => {
    sessionStorage.setItem("pedidoPesquisa", pesquisa)
  ...
  return (
    <div>
          ...
          <ul className="navbar-nav navbar-nav-right">
            <li className="nav-item nav-profile dropdown" >
              <div className='nav-link_1' onClick={() => Mostrar_opcao()}>
                ...
                <div className="destaque-alpha"> {primeiraLetraUsuario}</div>
              </div>
      ...
      {width < 600 && (
        <BarraLateral content3={showBarraLateral}></BarraLateral>
      )}
    </div>
  )
}
